...
for r=1:9
    ...
    %MixColumnResultTrue = conversion(MixColumnResultTrue);
    
    % Particular
    SboxTrans = sboxTrans(aF, aR);
    MixColumnResultTrans = mixcolumns(SboxTrans');
    mask = maskA(r,:);
    MixColumnResultTransMasked = bitxor(MixColumnResultTrans,maskA(r,:));
    ...
    % XOR
    XOR = bitxor(remasking, bitxor(NewkeyFake,remasking));
